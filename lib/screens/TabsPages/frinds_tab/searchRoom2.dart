...
class SearchRoomPage2 extends StatefulWidget {
  SearchRoomPage2();

  @override
  _SearchRoomPage2State createState() => _SearchRoomPage2State();
}

class _SearchRoomPage2State extends State<SearchRoomPage2> {
                  ...
                  children: <Widget>[
                    ...
                    _isLoading
                        ? FutureBuilder<SearchRoomModel>(
                            future: apiRequests.getSearchedmyrooms(_roomName),
                            builder: (context, snapshot) {
                              ...
                              if (snapshot.connectionState ==
                                ...
                                return  snapshot.data.data.roomsSearch.length==0? Center(
                                  child: Text("مفيش انهارده غور ",style: TextStyle(color: Color(int.parse(  _dataUser.getKey(Utility.FontColor),)),),),
                                ):GridView.count(
                                    ...
                                    children: List.generate(
                                      ...
                                      return GestureDetector(
                                          onTap: () {
                                            print(
                                                "---------------------------- active rooom------------");
                                            // print(snapshot.data.data.chatSearch[index].userImage);
                                            // print(snapshot.data.data.chatSearch[index].image);
                                            print(    snapshot.data.data.roomsSearch[index].roomName);
                                            print(snapshot.data.data.roomsSearch[index].roomId
                                      .toString());
                                            print(snapshot.data.data.roomsSearch[index]);
                                            Navigator.push(context,
                                                MaterialPageRoute(
                                                    builder: (context) {
                                              return ChatPage(
                                                   title:
                                     snapshot.data.data.roomsSearch[index].roomName,
